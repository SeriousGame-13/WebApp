...
import { Trophy, Users, Home, User, CalendarDays, Flame, Star, Moon, Settings, Search, Plus, Check, X, Dumbbell, Bike, HeartPulse, Activity, Info, Medal } from "lucide-react";
import './sphere-styles.css';

/**
 * THE SPHERE — Demo App (CSS version)
 * Part 1: Helper functions and basic components
 */
...
// Simple demo showing the components work
export default function Part1Demo() {
  const [modalOpen, setModalOpen] = useState(false);
  
  return (
    <Screen title="Component Demo" subtitle="Testing basic components">
      <div className="space-y-4">
        <Card>
          <div className="flex items-center gap-3">
            <Avatar name="Test User" size={40} />
            <div className="flex-1">
              <p className="font-semibold">Avatar Component</p>
              <p className="text-slate-400 text-sm">Generic avatar with gradient</p>
            </div>
          </div>
        </Card>
        ...
        <div className="grid-2 gap-4">
          <Card>
            <Stat label="TIME" value="32:10" />
          </Card>
          <Card>
            <Stat label="POINTS" value="84" />
          </Card>
        </div>

        <div className="flex gap-2">
          <Pill active={true}>Active Pill</Pill>
          <Pill active={false}>Inactive Pill</Pill>
        </div>
        ...
        <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Test Modal">
          <div className="space-y-3">
            <p className="text-slate-300">This is a test modal with basic styling.</p>
            <button 
...
import { HeartPulse, Dumbbell, Bike, Activity, Info } from "lucide-react";
import './sphere-styles.css';

// Import components from Part 1
import { Avatar, Modal, Card, Screen, Stat, secondsToClock } from './Part1Components';
...
export default function HomeDemo() {
  const handleStartWorkout = () => {
    alert("Start Workout clicked! (연결할 수 있음)");
  };
...
// Import components from Part 1
import { Avatar, Modal, Card, Screen } from './Part1Components';
...
const DEFAULT_GROUPS = [
  { 
    ...
    upcoming: [
      { when: "Tue 18:00", title: "Team HIIT" }, 
      { when: "Thu 18:00", title: "Plyo Flow" }
    ], 
  ...
  { 
    ...
    upcoming: [{ when: "Wed 19:00", title: "Intervals 6x800m" }], 
  ...
  { 
    ...
    upcoming: [{ when: "Sat 10:00", title: "Bootcamp" }], 
    memberIds: ["u3","u4"] 
  },
  { 
    id: "g4", 
    ...
    upcoming: [{ when: "Tue 20:00", title: "Team Challenge" }], 
...
// Import components from Part 1
import { Avatar, Card, Screen, Pill } from './Part1Components';
...
export default function RankingDemo() {
  const handlePreviewUser = (user) => {
    alert(`User Preview: ${user.name} (${user.points} points, Level ${user.level})`);
  };

  return <Ranking onPreviewUser={handlePreviewUser} />;
}

import React from "react";
import { Settings, Trophy, Users, CalendarDays, Flame, Star, Moon, Dumbbell, Bike, HeartPulse, Activity, Medal } from "lucide-react";
import './sphere-styles.css';

// Import components from Part 1
import { Avatar, Card, Screen } from './Part1Components';
...
const DUMMY_USER = {
  ...
  badges: [
    ...
    { id: "steps", name: "Step Up", level: 2, icon: <Activity className="w-5 h-5" /> },
  ],
};
...
export default function ProfileDemo() {
  const handleOpenBadge = (badgeId) => {
    alert(`Badge clicked: ${badgeId}`);
  };
...
import React, { useState } from "react";
import { Trophy, Users, Home, User, CalendarDays } from "lucide-react";
import './sphere-styles.css';

// Import all the screen components
import HomeDemo from './Part2Components';
import GroupsDemo from './Part3Components';
import RankingDemo from './Part4Components';
import ProfileDemo from './Part5Components';

// Simple Challenges component
function Challenges() {
  ...
  return (
    <div className="screen">
      ...
      <header className="screen-header">
        <h1 className="screen-title">Weekly Challenge</h1>
      </header>
            ...
            <div key={c.id} className="card">
              <div className="flex items-center gap-3">
                <div className={`${c.done ? "btn-primary" : "bg-white/10"} w-10 h-10 rounded-xl flex items-center justify-center`}>
                  {c.done ? "✓" : <CalendarDays className="w-5 h-5" />}
                </div>
                <div className="flex-1">
...
export default function App() {
  ...
  const renderCurrentTab = () => {
    switch(tab) {
      case "home": return <HomeDemo />;
      case "groups": return <GroupsDemo />;
      case "ranking": return <RankingDemo />;
      case "challenges": return <Challenges />;
      case "profile": return <ProfileDemo />;
      default: return <HomeDemo />;
    }
  };
